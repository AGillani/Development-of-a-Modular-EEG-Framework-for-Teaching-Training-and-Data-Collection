...
void loop(){
  ...
  while(digitalRead(ADS.DRDY)){}
   ADS.updatedata();
   accel();
   SerialBT.print((ADS.channelData[0]));
   SerialBT.println();
   /*
   //Pattern is ch1,ch2,ch3,ch4,ch5,ch6,ch7-a1,a2,a3.
   for(int i = 0; i<3; i++){
    SerialBT.print((int)(500000+MICROVOLTS_PER_COUNT* ADS.channelData[i]));
    SerialBT.print(',');
   } 
   SerialBT.print((int)(500000+MICROVOLTS_PER_COUNT* ADS.channelData[3]));
   SerialBT.print(',');
